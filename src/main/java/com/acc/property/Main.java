...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String fileName = "processedData.json";
			File file = new File(fileName);

			if (!file.exists()) {
				System.err.println("Error: processedData.json does not exist");
				sc.close();
				return;
			}
			...
			if (filteredResults.size() != 0) {
				...
				while (isContinue) {
					String searchString = Input.takeInput(freqCount, "Input word for frequency count", fileName,
							spellChecker);
			...
			System.out.println();
			System.out.println(
					"Steps for page ranking will be calculated based on number of bads and baths in a property: ");
